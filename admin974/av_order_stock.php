...
if (isset($_POST["Filtrer"]))
    $o = $db->rawQuery("select distinct id_order, firstname, lastname, c.id_customer, invoice_date
...
<div class="container">     
    ...
    <div class="row">
        ...
        <div class="col-xs-4">
            <a href="av_product_warehouse.php?PME_sys_operation=Ajouter" class="btn btn-primary">Ajouter un produit de stock</a>
    ...
    if ($o) {
        ...
        <div>
            ...
            foreach ($o as $order) {
                ...
                <h4><a href="av_orders_view.php?id_order=<?= $order["id_order"] ?>" target="_blank"><?= $order["id_order"] ?></a> - <a href="av_customer_view.php?id_customer=<?= $order["id_customer"] ?>" target="_blank"><?= $order["firstname"] ?> <?= $order["lastname"] ?></a> (<?= strftime("%d/%m/%y", strtotime($order["invoice_date"])) ?>)</h4> 
                ...
                <table class="table table-bordered table-condensed">
                    ...
                    <?
                    $params = array($order["id_order"], '%' . $order["id_order"] . '%', $current_state, $current_state, $invoice_stda_date, $invoice_stda_date, $invoice_enda_date);
                    ...
                    foreach ($r as $row) {
                        ...
                        <tr>
                            ...
                            <td style="width:100px" class="text-center <?= ($row["is_product_tracking"] == 1) ? 'alert-5' : 'alert-8' ?>">
                                <?= ($row["is_product_tracking"] == 1) ? '<span class="glyphicon glyphicon-ok">' : '<span class="glyphicon glyphicon-remove">' ?>
                            ...
                            <td style="width:100px" class="text-center"><?= $row["quantity_ordered"] ?></td>
                            <?
                            foreach ($w as $warehouse) {
                                $pws = $db->rawQuery("select * from av_product_warehouse where id_product = ? and id_warehouse = ?", array($row["id_product"], $warehouse["id_warehouse"]));
                                if ($pws) {
                                    foreach ($pws as $pw) {
                                        ?>
                                        <td style="width:100px" class="text-center <?= ($row["id_warehouse"] == $warehouse["id_warehouse"]) ? "alert-5" : "" ?> "><a href="av_product_warehouse.php?PME_sys_fl=1&PME_sys_fm=0&PME_sys_sfn[0]=0&PME_sys_operation=PME_op_Change&PME_sys_rec=<?= $pw["id_product_warehouse"] ?>"><?= $pw["quantity"] ?></a></td>
                                        <?
                                    }
                                } else {
                                    ?>
